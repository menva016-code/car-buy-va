...
export function InteriorPhotosForm({ ownerData, vehicleData, appraisalId: existingId, onBack, onSuccess }: InteriorPhotosFormProps) {
  ...
  if (submitted) {
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center px-6 ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
        <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-3xl p-8 shadow-sm w-full max-w-md text-center`}>
          ...
          <h2 className={`text-[20px] font-semibold mb-2 ${isDark ? 'text-white' : 'text-ink'}`}>Оценка сохранена</h2>
          <p className={`text-[15px] ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Данные и фотографии салона успешно переданы в систему</p>
          <button
            type="button"
            onClick={onSuccess}
            className="mt-6 w-full bg-brand-500 hover:bg-brand-600 text-white font-semibold py-3.5 rounded-2xl transition-colors"
          >
            На главную
          </button>
        </div>
